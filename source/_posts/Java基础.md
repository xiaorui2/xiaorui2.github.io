...
- 两者都是抽象类，都不能实例化。
- interface实现类及 abstrct class 的子类都必须要实现已经声明的抽象方法。
...
- 接口实现类必须全部实现抽象方法，而抽象类不用。

应用场景：在既需要统一的接口，又需要实例变量或缺省的方法的情况下，就可以使用抽象类。其他适用的情况下我更常用接口实现。
...
因为很多方法参数就只接收对象类型(Object)，比如范型就只支持对象类型以及如果在数据库操作中某数值列允许为 null 的话，那接受该列数据的类型就只能为 Integer
...
- StringBuffer 字符串变量（线程安全）其 toString 方法会进行对象缓存，以减少元素复制开销，支持同步锁。它的线程安全是在 append 方法上加了 synchronized 关键字来实现同步操作。
...
//它与纯粹地使用StringBuilder的append方法是不同的： 每趟循环都会创建一个StringBuilder对象，每次执行完毕都会调用toString方法将其转换为字符串
```

对于 concat 整体是一个数组的拷贝，虽然在内存中是处理都是原子性操作，速度非常快，但是，最后的 return 语句创建一个新 String 对象，也就是每次 concat 操作都会创建一个新的 String 对象，[源码看这](https://blog.csdn.net/zen99t/article/details/51255418)
...
Java 当修改 final 对象、常量值时要重新编译类。静态常量（即用 static final 修饰的变量）是编译时常量，当一个class文件编译完毕，它内部使用到的所有常量的具体值就已经确定了
...
// 所以单纯的修改静态变量是没用的，还要重新编辑所有引用静态变量的类，不然不会生效的。
```

## 不用final还可以用什么办法使得这个类不被继承

如果类中只有 private 的构造方法，那么此类不可以被继承。
...
就是 PriorityQueue ，它默认是小顶堆，重写 compare 就可以实现大顶堆。
...
通过反射创建类对象：通过 Class 对象的 newInstance() 方法。
...
第二段代码则是在运行时通过字符串值才得知要运行的类（com.chenshuyi.reflect.Apple）

反射中最最终的就是 Method 类的 invoke 方法了，也就是我们说的原理。这个方法究竟做了哪些事情？

一开始是进行了一些权限的检查，最后是调用了 MethodAccessor 类的 invoke 方法。MethodAccessor就是上面提到的所有同名method共享的一个实例，由ReflectionFactory创建。创建机制采用了一种名为inflation的方式（JDK1.4之后）：如果该方法的累计调用次数<=15，会创建出NativeMethodAccessorImpl，它的实现就是直接调用native方法实现反射；如果该方法的累计调用次数>15，会由java代码创建出字节码组装而成的MethodAccessorImpl。

# Throwable、error、exception的区别

异常本质上是程序上的错误，包括程序逻辑错误和系统错误。比如使用空的引用、数组下标越界、内存溢出错误等。Throwable 类是 Java 语言中所有错误或异常的超类。有两个重要的子类：Exception（异常）和 Error（错误），二者都是 Java 异常处理的重要子类，各自都包含大量子类。
...
怎么 catch？catch throwable 会把 Error 和其他继承 Throwable 的类捕捉到。而 catch Exception 只会捕捉Exception及其子类，捕捉的范围更小。

Error：是程序无法处理的错误，表示运行应用程序中较严重问题。大多数错误与代码编写者执行的操作无关，而表示代码运行时 JVM 出现的问题。 

## 如何处理异常

在 Java 中如果需要处理异常，必须利用 try catch 先对异常进行捕获，然后再用到 throw 和 throws 关键字对异常情况进行处理。

## throw 和 throws 关键字

throws 出现在方法的声明中，表示该方法可能会抛出的异常，然后交给上层调用它的方法程序处理，允许 throws后面跟着多个异常类型；

一般会用于程序出现某种逻辑时程序员主动抛出某种特定类型的异常。throw 只会出现在方法体中，当方法在执行过程中遇到异常情况时，将异常信息封装为异常对象，然后 throw 出去。throw 关键字的一个非常重要的作用就是 异常类型的转换。

throws 表示出现异常的一种可能性，并不一定会发生这些异常；throw 则是抛出了异常，执行 throw 则一定抛出了某种异常对象。两者都只是抛出或者可能抛出异常，但是不会由方法去处理异常，真正的处理异常由此方法的上层调用处理。
...
## yield() 
yield() 方法和 sleep() 方法类似，也不会释放“锁标志”，区别在于，它没有参数，即 yield() 方法只是使当前线程使当前线程从执行状态（运行状态）变为可执行状态（就绪状态），所以执行 yield() 的线程有可能在进入到可执行状态后马上又被执行，另外 yield() 方法只能使同优先级或者高优先级的线程得到执行机会，这也和 sleep() 方法不同。
